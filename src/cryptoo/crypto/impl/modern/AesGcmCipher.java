...
public class AesGcmCipher implements TextCipher {
    ...
    public String algoCode() {
        return cryptoo.CryptoUtils.ALGO_AES_GCM;
    }
    ...
    private byte[] deriveAesKeyBytes(String keyStr) throws Exception {
        ...
        if (looksLikeBase64Key(trimmed)) {
            ...
            // 128 bit (16), 192 (24), 256 (32) encoded key support
            // But if it's a password, we force 128 below.
            if (raw.length == 16 || raw.length == 24 || raw.length == 32) {
                return raw;
            }
        }

        // PBKDF2 -> Force 128-bit (16 bytes) as per homework requirement
